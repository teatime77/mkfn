...
public:
	...
	void SetBatchData(T* X, T* batch_X, T* batch_Y, UCHAR* label, int batch_size, int* idxes, int mini_batch_idx) {
		for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
			...
			void* src = &X[idx];
			void* dst = &batch_X[batch_idx * DomainLen];

			memcpy(dst, src, sizeof(T) * DomainLen);

			batch_Y[batch_idx * RangeLen + label[idx]] = 1;
		}
	}
