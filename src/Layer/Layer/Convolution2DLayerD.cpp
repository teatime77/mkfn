...
#include "MkFn.h"
#include "../../Lib/Lib.h"
#include "Convolution2DLayerD.h"

void Convolution2DLayerD::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int Convolution2DLayerD::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
int* Convolution2DLayerD::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
void Convolution2DLayerD::GetFieldValue(int field_idx, void* dst){
...
void Convolution2DLayerD::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
Convolution2DLayerD::Convolution2DLayerD(int m_size, int n_size, int k_size, int cm_size, int cn_size){
...
Convolution2DLayerD::~Convolution2DLayerD(){
	Free();
	_Free(h);
	_Free(b);
...
void Convolution2DLayerD::Allocate(){
...
void Convolution2DLayerD::Forward(){
...
void Convolution2DLayerD::Backward(){
#pragma omp parallel for
    for (int i = 0; i < (M - CM) + 1; i++) {
        for (int j = 0; j < (N - CN) + 1; j++) {
            for (int k = 0; k < K; k++) {
...
void Convolution2DLayerD::UpdateParameter_0(){
	int _count = CM * CN * K;
#pragma omp parallel for
...
void Convolution2DLayerD::UpdateParameter_1(){
	int _count = K;
#pragma omp parallel for
	for(int _idx = 0; _idx < _count; _idx++) {
		int offset = _idx * BatchSize;
...
void Convolution2DLayerD::UpdateParameter(){
	UpdateParameter_0();
	UpdateParameter_1();
}

extern "C" DllExport Layer* MakeConvolution2DLayerD(int m_size, int n_size, int k_size, int cm_size, int cn_size){
	return new Convolution2DLayerD(m_size, n_size, k_size, cm_size, cn_size);
}
