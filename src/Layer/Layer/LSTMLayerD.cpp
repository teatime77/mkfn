...
#include "MkFn.h"
#include "../../Lib/Lib.h"
#include "LSTMLayerD.h"

void LSTMLayerD::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int LSTMLayerD::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
int* LSTMLayerD::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
void LSTMLayerD::GetFieldValue(int field_idx, void* dst){
...
void LSTMLayerD::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
LSTMLayerD::LSTMLayerD(int t_size, int x_size, int y_size){
...
void LSTMLayerD::UpdateParameter_0(){
	int _count = Y * X;
#pragma omp parallel for
...
void LSTMLayerD::UpdateParameter_1(){
	int _count = Y * Y;
#pragma omp parallel for
	for(int _idx = 0; _idx < _count; _idx++) {
		int offset = _idx * BatchSize;
...
void LSTMLayerD::UpdateParameter_2(){
	int _count = Y;
#pragma omp parallel for
	for(int _idx = 0; _idx < _count; _idx++) {
		int offset = _idx * BatchSize;
...
void LSTMLayerD::UpdateParameter(){
	UpdateParameter_0();
	UpdateParameter_1();
	UpdateParameter_2();
}
...
extern "C" DllExport Layer* MakeLSTMLayerD(int t_size, int x_size, int y_size){
	return new LSTMLayerD(t_size, x_size, y_size);
}
