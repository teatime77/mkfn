...
#include "MkFn.h"
#include "../../Lib/Lib.h"
#include "RecurrentLayerD.h"

void RecurrentLayerD::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int RecurrentLayerD::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
int* RecurrentLayerD::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
void RecurrentLayerD::GetFieldValue(int field_idx, void* dst){
...
void RecurrentLayerD::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
RecurrentLayerD::RecurrentLayerD(int t_size, int x_size, int y_size){
...
RecurrentLayerD::~RecurrentLayerD(){
	Free();
	_Free(win);
	_Free(w);
	_Free(b);
...
void RecurrentLayerD::UpdateParameter_0(){
	int _count = Y * X;
#pragma omp parallel for
...
void RecurrentLayerD::UpdateParameter_1(){
	int _count = Y * Y;
#pragma omp parallel for
	for(int _idx = 0; _idx < _count; _idx++) {
		int offset = _idx * BatchSize;
...
void RecurrentLayerD::UpdateParameter_2(){
	int _count = Y;
#pragma omp parallel for
	for(int _idx = 0; _idx < _count; _idx++) {
		int offset = _idx * BatchSize;
...
void RecurrentLayerD::UpdateParameter(){
	UpdateParameter_0();
	UpdateParameter_1();
	UpdateParameter_2();
}
...
extern "C" DllExport Layer* MakeRecurrentLayerD(int t_size, int x_size, int y_size){
	return new RecurrentLayerD(t_size, x_size, y_size);
}
