...
#include "MkFn.h"
#include "../../Lib/Lib.h"
#include "FullyConnectedLayerF.h"

void FullyConnectedLayerF::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int FullyConnectedLayerF::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 1;
...
int* FullyConnectedLayerF::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2:
...
void FullyConnectedLayerF::GetFieldValue(int field_idx, void* dst){
...
void FullyConnectedLayerF::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
FullyConnectedLayerF::FullyConnectedLayerF(int x_size, int y_size){
...
FullyConnectedLayerF::~FullyConnectedLayerF(){
	Free();
	_Free(w);
	_Free(b);
...
void FullyConnectedLayerF::Allocate(){
...
void FullyConnectedLayerF::Free(){
	_chk(_Free(y)); 
	_chk(_Free(u)); 
	_chk(_Free(delta_x)); 
	_chk(_Free(delta_w)); 
...
void FullyConnectedLayerF::Forward(){
...
void FullyConnectedLayerF::Backward(){
#pragma omp parallel for
    for (int i = 0; i < Y; i++) {
        for (int _batch_idx = 0; _batch_idx < BatchSize; _batch_idx++) {
...
void FullyConnectedLayerF::UpdateParameter_0(){
	int _count = Y * X;
#pragma omp parallel for
...
void FullyConnectedLayerF::UpdateParameter_1(){
	int _count = Y;
#pragma omp parallel for
	for(int _idx = 0; _idx < _count; _idx++) {
		int offset = _idx * BatchSize;
...
void FullyConnectedLayerF::UpdateParameter(){
	UpdateParameter_0();
	UpdateParameter_1();
}

extern "C" DllExport Layer* MakeFullyConnectedLayerF(int x_size, int y_size){
	return new FullyConnectedLayerF(x_size, y_size);
}
