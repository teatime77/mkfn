...
#include "../../Lib/Lib.h"
#include "LibCuda.h"
#include "RecurrentLayerCudaF.h"
__constant__ int _BatchSize;
__constant__ float _LearningRate;

void RecurrentLayerCudaF::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int RecurrentLayerCudaF::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
int* RecurrentLayerCudaF::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
void RecurrentLayerCudaF::GetFieldValue(int field_idx, void* dst){
...
void RecurrentLayerCudaF::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
RecurrentLayerCudaF::RecurrentLayerCudaF(int t_size, int x_size, int y_size){
...
RecurrentLayerCudaF::~RecurrentLayerCudaF(){
	Free();
...
void RecurrentLayerCudaF::Allocate(){
...
__global__ static void forward_u(float* u, int X, float* x, float* win, int Y, float* w, float* y, float* b, int T, int t){
...
void RecurrentLayerCudaF::Start_forward_u(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = Y;
...
__global__ static void forward_y(float* y, float* u, int T, int Y, int t){
	int _batch_idx = threadIdx.x;
	int j = blockIdx.x;
...
void RecurrentLayerCudaF::Start_forward_y(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = Y;
...
void RecurrentLayerCudaF::Forward(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
	Start_forward_u();
	Start_forward_y();
}
__global__ static void backward_delta_y(float* delta_y, int Y, float* delta_u, float* w, int T, int t){
	int _batch_idx = threadIdx.x;
	int j = blockIdx.x;
...
void RecurrentLayerCudaF::Start_backward_delta_y(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = Y;
...
__global__ static void backward_delta_u(float* delta_u, float* delta_y, float* u, int T, int Y, int t){
	int _batch_idx = threadIdx.x;
	int j = blockIdx.x;
...
void RecurrentLayerCudaF::Start_backward_delta_u(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = Y;
...
__global__ static void backward_delta_x(float* delta_x, int Y, float* delta_u, float* win, int T, int X, int t){
	int _batch_idx = threadIdx.x;
	int i_i = blockIdx.x;
...
void RecurrentLayerCudaF::Start_backward_delta_x(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = X;
...
void RecurrentLayerCudaF::Start_backward_delta_win(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_y = Y;
...
__global__ static void backward_delta_w(float* delta_w, int T, float* delta_u, float* y, int Y, int t){
	int _batch_idx = threadIdx.x;
	int j = blockIdx.y;
	int i_i = blockIdx.x;
...
void RecurrentLayerCudaF::Start_backward_delta_w(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_y = Y;
...
void RecurrentLayerCudaF::Start_backward_delta_b(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = Y;
...
void RecurrentLayerCudaF::Backward(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
...
__global__ static void UpdateParameterKernel_0(float* win, float* delta_win){
...
void RecurrentLayerCudaF::UpdateParameter_0(){
	int threads_x = 1;
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
...
__global__ static void UpdateParameterKernel_1(float* w, float* delta_w){
	int _idx = (blockIdx.x) * blockDim.x + threadIdx.x;
	int offset = _idx * _BatchSize;
	{
		float sum = 0;
...
void RecurrentLayerCudaF::UpdateParameter_1(){
	int threads_x = 1;
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
...
__global__ static void UpdateParameterKernel_2(float* b, float* delta_b){
...
void RecurrentLayerCudaF::UpdateParameter_2(){
	int threads_x = 1;
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
...
void RecurrentLayerCudaF::UpdateParameter(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
...
extern "C" DllExport Layer* MakeRecurrentLayerCudaF(int t_size, int x_size, int y_size){
	return new RecurrentLayerCudaF(t_size, x_size, y_size);
}
