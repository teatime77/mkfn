...
#include "../../Lib/Lib.h"
#include "LibCuda.h"
#include "MaxPooling2DLayerCudaD.h"
__constant__ int _BatchSize;
__constant__ double _LearningRate;

void MaxPooling2DLayerCudaD::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int MaxPooling2DLayerCudaD::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
int* MaxPooling2DLayerCudaD::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
void MaxPooling2DLayerCudaD::GetFieldValue(int field_idx, void* dst){
...
void MaxPooling2DLayerCudaD::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
MaxPooling2DLayerCudaD::MaxPooling2DLayerCudaD(int m_size, int n_size, int k_size, int pm_size, int pn_size){
...
MaxPooling2DLayerCudaD::~MaxPooling2DLayerCudaD(){
	Free();
...
void MaxPooling2DLayerCudaD::Allocate(){
...
void MaxPooling2DLayerCudaD::Free(){
	_chk(_Free(y)); 
	_chk(_Free(delta_x)); 
	_chk(_Free(_idx_x)); 
}
__global__ static void forward_y(double* y, int PM, int PN, double* x, int* _idx_x, int MH, int NH, int K, int M, int N){
...
void MaxPooling2DLayerCudaD::Start_forward_y(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_z = MH;
...
void MaxPooling2DLayerCudaD::Forward(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
	Start_forward_y();
}
__global__ static void backward_delta_x(double* delta_x, double* delta_y, int PM, int PN, double* x, double* y, int* _idx_x, int M, int N, int K, int MH, int NH){
	int _batch_idx = threadIdx.x;
	int i_0 = blockIdx.z;
	int i_1 = blockIdx.y;
	int k = blockIdx.x;
...
void MaxPooling2DLayerCudaD::Start_backward_delta_x(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_z = M;
...
void MaxPooling2DLayerCudaD::Backward(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
	Start_backward_delta_x();
}
void MaxPooling2DLayerCudaD::UpdateParameter(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
...
extern "C" DllExport Layer* MakeMaxPooling2DLayerCudaD(int m_size, int n_size, int k_size, int pm_size, int pn_size){
	return new MaxPooling2DLayerCudaD(m_size, n_size, k_size, pm_size, pn_size);
}
