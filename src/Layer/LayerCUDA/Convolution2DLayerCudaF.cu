...
#include "../../Lib/Lib.h"
#include "LibCuda.h"
#include "Convolution2DLayerCudaF.h"
__constant__ int _BatchSize;
__constant__ float _LearningRate;

void Convolution2DLayerCudaF::GetFieldName(int field_idx, wchar_t* name){
	switch(field_idx){
...
int Convolution2DLayerCudaF::GetFieldDimension(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
int* Convolution2DLayerCudaF::GetFieldSize(int field_idx){
	switch(field_idx){
	case 0: return 0;
	case 1: return 0;
	case 2: return 0;
...
void Convolution2DLayerCudaF::GetFieldValue(int field_idx, void* dst){
...
void Convolution2DLayerCudaF::SetFieldValue(int field_idx, void* src){
	int _cnt = GetFieldElementCount(field_idx);
	switch(field_idx){
...
Convolution2DLayerCudaF::Convolution2DLayerCudaF(int m_size, int n_size, int k_size, int cm_size, int cn_size){
...
Convolution2DLayerCudaF::~Convolution2DLayerCudaF(){
	Free();
...
void Convolution2DLayerCudaF::Allocate(){
...
void Convolution2DLayerCudaF::Free(){
	_chk(_Free(y)); 
	_chk(_Free(u)); 
	_chk(_Free(delta_x)); 
	_chk(_Free(delta_u)); 
...
__global__ static void forward_u(float* u, int CM, int CN, float* x, float* h, float* b, int M, int N, int K){
...
void Convolution2DLayerCudaF::Start_forward_u(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_z = (M - CM) + 1;
...
__global__ static void forward_y(float* y, float* u, int M, int CM, int N, int CN, int K){
	int _batch_idx = threadIdx.x;
	int i = blockIdx.z;
	int j = blockIdx.y;
	int k = blockIdx.x;
...
void Convolution2DLayerCudaF::Start_forward_y(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_z = (M - CM) + 1;
...
void Convolution2DLayerCudaF::Forward(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
	Start_forward_u();
	Start_forward_y();
}
__global__ static void backward_delta_u(float* delta_u, float* delta_y, float* u, int M, int CM, int N, int CN, int K){
	int _batch_idx = threadIdx.x;
	int i = blockIdx.z;
	int j = blockIdx.y;
	int k = blockIdx.x;
...
void Convolution2DLayerCudaF::Start_backward_delta_u(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_z = (M - CM) + 1;
...
__global__ static void backward_delta_x(float* delta_x, int K, int M, int CM, int N, int CN, float* delta_u, float* h){
	int _batch_idx = threadIdx.x;
	int i_0 = blockIdx.y;
	int i_1 = blockIdx.x;
...
void Convolution2DLayerCudaF::Start_backward_delta_x(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_y = M;
...
__global__ static void backward_delta_h(float* delta_h, int M, int CM, int N, int CN, float* delta_u, float* x, int K){
	int _batch_idx = threadIdx.x;
	int i_p = blockIdx.z;
	int i_q = blockIdx.y;
	int k = blockIdx.x;
...
void Convolution2DLayerCudaF::Start_backward_delta_h(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_z = CM;
...
__global__ static void backward_delta_b(float* delta_b, int M, int CM, int N, int CN, float* delta_u, int K){
	int _batch_idx = threadIdx.x;
	int k = blockIdx.x;
...
void Convolution2DLayerCudaF::Start_backward_delta_b(){
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
	blocks_x = K;
...
void Convolution2DLayerCudaF::Backward(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
...
__global__ static void UpdateParameterKernel_0(float* h, float* delta_h){
...
void Convolution2DLayerCudaF::UpdateParameter_0(){
	int threads_x = 1;
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
...
void Convolution2DLayerCudaF::UpdateParameter_1(){
	int threads_x = 1;
	int blocks_x = 1;
	int blocks_y = 1;
	int blocks_z = 1;
...
void Convolution2DLayerCudaF::UpdateParameter(){
	_chk(_MemcpyToSymbol(_BatchSize, BatchSize, sizeof(BatchSize)));
...
extern "C" DllExport Layer* MakeConvolution2DLayerCudaF(int m_size, int n_size, int k_size, int cm_size, int cn_size){
	return new Convolution2DLayerCudaF(m_size, n_size, k_size, cm_size, cn_size);
}
